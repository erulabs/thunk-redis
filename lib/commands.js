...
exports.initCommands = function (proto) {
  ...
  proto.xread = function () {
    return sendCommand(this, 'xread', adjustArgs(arguments), 0, streamResponseToHash)
  }
...
function streamResponseToHash (array) {
  if (!array) return
  const hash = {}

  for (let i = 0; i < array.length; i += 1) {
    const stream = array[i]
    hash[stream[0]] = stream[1]
  }
